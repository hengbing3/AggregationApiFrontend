...
export type Props = {
  columns: ProColumns<API.InterfaceInfoApplyParam>[];
  // 当用户点击取消按钮时触发
  onCancel: () => void;
  // 当用户提交表单时,将用户输入的数据作为参数传递给后台
  onSubmit: (values: API.InterfaceInfoApplyParam) => Promise<void>;
  // 模态框是否可见
  visible: boolean;
  // values不用传递
  ...
  return (
    ...
    >
      <ProTable
        ...
        columns={columns}
        dataSource={[]}
        onSubmit={async (valuse: API.InterfaceInfoApplyParam) => {
