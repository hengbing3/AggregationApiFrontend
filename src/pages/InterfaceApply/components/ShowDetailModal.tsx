 ...
 * @LastEditTime: 2024-03-24 20:11:25
...
export type Props = {
  values: Partial<API.InterfaceInfoApply>;
  columns: ProColumns<API.InterfaceInfoApply>[];
  // 当用户点击取消按钮时触发
  onCancel: () => void;
  // 当用户提交表单时,将用户输入的数据作为参数传递给后台
  onSubmit: (values: API.InterfaceInfoApply) => Promise<void>;
  // 模态框是否可见
  visible: boolean;
};
const ShowDetailModal: React.FC<Props> = (props) => {
  ...
  return (
    <Modal visible={visible} footer={null} onCancel={() => onCancel?.()} width={800}>
      <ProTable
        type="form"
        formRef={formRef}
        columns={columns}
        onSubmit={async (valuse: API.InterfaceInfoApply) => {
          onSubmit?.(valuse);
        }}
        onReset={() => {
          onCancel?.();
