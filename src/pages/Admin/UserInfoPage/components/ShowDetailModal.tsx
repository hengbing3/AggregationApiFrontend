 ...
 * @LastEditTime: 2024-03-25 22:35:22
...
export type Props = {
  values: Partial<API.UserInfoVO>;
  columns: ProColumns<API.UserInfoVO>[];
  // 当用户点击取消按钮时触发
  onCancel: () => void;
...
const ShowDetailModal: React.FC<Props> = (props) => {
  ...
  return (
    <Modal visible={visible} footer={null} onCancel={() => onCancel?.()} width={600}>
      <ProTable
        type="form"
        formRef={formRef}
        columns={columns}
        onSubmit={async (valuse: API.UserInfoVO) => {
          onSubmit?.(valuse);
        }}
        onReset={() => {
          onCancel?.();
