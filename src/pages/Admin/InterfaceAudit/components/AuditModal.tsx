 ...
 * @LastEditTime: 2024-03-24 22:17:47
...
export type Props = {
  values: Partial<API.InterfaceInfoApplyAudit>;
  columns: ProColumns<API.InterfaceInfoApplyAudit>[];
  // 当用户点击取消按钮时触发
  onCancel: () => void;
  // 当用户提交表单时,将用户输入的数据作为参数传递给后台
  onSubmit: (values: API.InterfaceInfoApplyAudit) => Promise<void>;
  // 模态框是否可见
  visible: boolean;
};
const AuditModal: React.FC<Props> = (props) => {
  ...
  return (
    ...
    >
      <ProTable
        ...
        formRef={formRef}
        columns={columns}
        onSubmit={async (valuse: API.InterfaceInfoApplyAudit) => {
          onSubmit?.(valuse);
          handleReset();
        }}
      />
